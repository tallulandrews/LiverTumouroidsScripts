...
predictor<-predictor[keep];
surv_test1 <- coxph(surv_tcga_obj~predictor) # Continuous predictor?
surv_test2 <- survreg(surv_tcga_obj~predictor, dist="logistic") # Continuous predictor
...
sng_gene_test <- function(x) {
	...
	surv_tcga_obj <- Surv(times, dead)
	surv_test1 <- coxph(surv_tcga_obj~predictor) # Cox proportional hazards model 
#	surv_test2 <- survreg(surv_tcga_obj~predictor, dist="logistic") # Continuous predictor
...
tcga_obj <- list(CCA1=CCA1_markers, HCC6=HCC6_markers, HCC10=HCC10_markers)
saveRDS(tcga_obj, "Laura_Complex_Markers.rds")
...
tcga_obj <- list(CCA1=CCA1_markers, HCC6=HCC6_markers, HCC10=HCC10_markers)
saveRDS(tcga_obj, "Laura_Strict_Complex_Markers.rds")
require("scater")
...
check_assignment <- function(marker_tcga_obj, ext_markers, SCE) {
	assign <- marker_tcga_obj[,-c(1,ncol(marker_tcga_obj), ncol(marker_tcga_obj)-1)]
	assign <- assign[marker_tcga_obj$q.value < 0.05,]
