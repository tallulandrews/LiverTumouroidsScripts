...
intensity_plot_plusbar <- function(SCE, gene) {
	layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
        vals <- exprs(SCE)[fData(SCE)$feature_symbol == gene,];
        ...
        my_cols <- colorRampPalette(brewer.pal(6, "Blues"))(length(levels(bins)));
        plot(SCE$dm1, SCE$dm2, col=my_cols[bins], pch=16, main=gene, xaxt="n", yaxt="n", xlab="", ylab="", bty="n")
	color.bar(my_cols, ticks.lab=round(splits, digits=1), add=TRUE)
}

png("ForPascal_CCA1_NDRG1.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "NDRG1")
dev.off()
png("ForPascal_CCA1_DAPK1.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "DAPK1")
dev.off()
png("ForPascal_CCA1_CA9.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "CA9")
dev.off()
png("ForPascal_HCC10_G6PC.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "G6PC")
dev.off()
png("ForPascal_HCC10_CA9.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "CA9")
dev.off()
png("ForPascal_HCC10_RDH16.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "RDH16")
dev.off()

png("ForPascal_HCC10_MKI67.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "MKI67")
dev.off()
png("ForPascal_HCC10_HMGB2.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "HMGB2")
dev.off()
png("ForPascal_HCC10_HMMR.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "HMMR")
dev.off()
png("ForPascal_HCC10_TTK.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(HCC10, "TTK")
dev.off()
png("ForPascal_CCA1_MKI67.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "MKI67")
dev.off()
png("ForPascal_CCA1_HMGB2.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "HMGB2")
dev.off()
png("ForPascal_CCA1_HMMR.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "HMMR")
dev.off()
png("ForPascal_CCA1_TTK.png", width=5, height=5, units="in", res=300)
intensity_plot_plusbar(CCA1, "TTK")
dev.off()
...
dm_dims <- cbind(pData(CCA1)$dm1, pData(CCA1)$dm2)
cluster_lab <- as.character(pData(CCA1)$sc3_6_clusters)
cluster_lab[!cluster_lab %in% names(CCA1_colours)] <- "Outliers"
cluster_lab <- factor(cluster_lab, levels=names(CCA1_colours));

png("ForPascal_CCA1_Pseudotime_onwhite.png", width=5, height=5, units="in", res=300)
layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
par(mar=c(4,4,1,1))
plot(dm_dims[,1], dm_dims[,2], bg=CCA1_colours[cluster_lab], xlab="Dimension 1", ylab="Dimension 2", pch=21);
blank_plot()
legend("left", names(CCA1_colours), pt.bg=CCA1_colours, pch=21,bty="n")
dev.off()

cc <- readRDS("CCA1_SC3_Prolif.rds")
cc <- cc$Proliferating +1
png("ForPascal_CCA1_Pseudotime_CC_onwhite.png", width=5, height=5, units="in", res=300)
layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
par(mar=c(4,4,1,1))
plot(dm_dims[,1], dm_dims[,2], col=c("black","red")[cc], xlab="Dimension 1", ylab="Dimension 2", pch=16);
blank_plot()
legend("left", title="Cycling", c("Y", "N"), col=c("red", "black"), pch=16,bty="n")
dev.off()

png("ForPascal_CCA1_Pseudotime_ongrey.png", width=5, height=5, units="in", res=300)
par(mar=c(4,4,1,1))
layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
plot(dm_dims[,1], dm_dims[,2], bg=CCA1_colours[cluster_lab], xlab="Dimension 1", ylab="Dimension 2", pch=21);
rect(min(dm_dims[,1])-2, min(dm_dims[,2])-2, max(dm_dims[,1])+2, max(dm_dims[,2])+2, col=grey_bg)
par(new=TRUE)
plot(dm_dims[,1], dm_dims[,2], col=CCA1_colours[cluster_lab], xlab="Dimension 1", ylab="Dimension 2", pch=16);
par(new=FALSE)
blank_plot()
legend("left", names(CCA1_colours), col=CCA1_colours, pch=16,bg=grey_bg)
dev.off()
table(cluster_lab)



dm_dims <- cbind(pData(HCC10)$dm1, pData(HCC10)$dm2)
cluster_lab <- as.character(pData(HCC10)$sc3_6_clusters)
cluster_lab[!cluster_lab %in% names(HCC10_colours)] <- "Outliers"
cluster_lab <- factor(cluster_lab, levels=names(HCC10_colours));

png("ForPascal_HCC10_Pseudotime_onwhite.png", width=5, height=5, units="in", res=300)
layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
par(mar=c(4,4,1,1))
plot(dm_dims[,1], dm_dims[,2], bg=HCC10_colours[cluster_lab], xlab="Dimension 1", ylab="Dimension 2", pch=21);
blank_plot()
legend("left", names(HCC10_colours), pt.bg=HCC10_colours, pch=21,bty="n")
dev.off()

cc <- readRDS("HCC10_SC3_Prolif.rds")
cc <- cc$Proliferating +1
png("ForPascal_HCC10_Pseudotime_CC_onwhite.png", width=5, height=5, units="in", res=300)
layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
par(mar=c(4,4,1,1))
plot(dm_dims[,1], dm_dims[,2], col=c("black","red")[cc], xlab="Dimension 1", ylab="Dimension 2", pch=16);
blank_plot()
...
png("ForPascal_HCC10_Pseudotime_ongrey.png", width=5, height=5, units="in", res=300)
par(mar=c(4,4,1,1))
layout(matrix(c(1,2,1,2), byrow=T, ncol=2), widths=c(5,1), heights=c(1,1))
plot(dm_dims[,1], dm_dims[,2], bg=HCC10_colours[cluster_lab], xlab="Dimension 1", ylab="Dimension 2", pch=21);
rect(min(dm_dims[,1])-2, min(dm_dims[,2])-2, max(dm_dims[,1])+2, max(dm_dims[,2])+2, col=grey_bg)
par(new=TRUE)
plot(dm_dims[,1], dm_dims[,2], col=HCC10_colours[cluster_lab], xlab="Dimension 1", ylab="Dimension 2", pch=16);
par(new=FALSE)
blank_plot()
legend("left", names(HCC10_colours), col=HCC10_colours, pch=16,bg=grey_bg)
dev.off()
table(cluster_lab)
